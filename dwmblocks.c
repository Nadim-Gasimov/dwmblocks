...
void getcmd(const Block *block, char *output)
{
    ...
    // TODO decide whether its better to use the last value till next time or
    // just keep trying while the error was the interrupt this keeps trying to
    // read if it got nothing and the error was an interrupt
    ...
    //  updated again
    // either way you have to save the data to a temp buffer because when it
    // fails it writes nothing and then then it gets displayed before this
    // finishes
    char *s;
    int e;
    do
...
/*    int button = si->si_value.sival_int;  // Если кнопка 0, то сигнал не от
 * нажатия кнопки*/
/*    switch (signal) {*/
/*        case 1:*/
/*            if (button == 1) {*/
/*                // Левый клик: выполнить действие для первого блока*/
/*                system("st -e bash -c \"less -Sf
 * /home/nadim/.config/dwmblocks/wifi_status.sh; read\"");*/
/*            } else if (button == 2) {*/
/*                // Средний клик: обновить прогноз погоды*/
/*                system("~/.local/bin/sb-forecast");*/
/*            }*/
/*            break;*/
/*        case 2:*/
/*            // Можно добавить другие действия для других сигналов*/
/*            break;*/
/*        default:*/
/*            break;*/
/*    }*/
...
void buttonhandler(int ssi_int)
{
    ...
    if (fork() == 0)
    {
        ...
        char shcmd[1024];
        sprintf(shcmd, "%s && kill -%d %d", current->command,
                current->signal + 34, process_id);
